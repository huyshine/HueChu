...
function update_user($user_id, $username, $password, $hovaten, $email, $tel, $vaitro_id)
{
    include '../ketnoi/ketnoi.php';
    $errors = [];
    if ($password == "") {
    ...
    $_SESSION['errors'] =  $errors;
    if (!$errors) {
        $sql = "UPDATE taikhoan SET user_id = '$user_id',username='$username',password = '$password',hovaten='$hovaten',email='$email',tel='$tel',vaitro_id='$vaitro_id'WHERE user_id = '$user_id'";
        $stmt = $conn->prepare($sql);
        $stmt->execute();
    }
}
...
{
    include '../ketnoi/ketnoi.php';
    $sql = "SELECT taikhoan.hovaten,taikhoan.email,taikhoan.tel,tbl_order.user_id, COUNT(tbl_order.user_id) as 'solanmua' FROM tbl_order JOIN taikhoan ON taikhoan.user_id = tbl_order.user_id GROUP by tbl_order.user_id limit 3";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
